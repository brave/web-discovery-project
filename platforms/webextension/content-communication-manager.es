...
export default class ContentCommunicationManager {
  ...
  callContentAction(module, action, { windowId, url, frameId }, ...args) {
    ...
    return Promise.reject(
      new Error(
        'callContentAction expects either "url" or "windowId" to be specified'
      )
    );
  }
  ...
  async broadcastToUrl(url, msg) {
    const candidates = (await browser.tabs.query({})).filter((tab) =>
      urlEquals(tab.url, url)
    );
    ...
    return Promise.all(
      candidates.map(({ id }) =>
        browser.tabs.sendMessage(id, msg, { frameId: 0 })
      )
