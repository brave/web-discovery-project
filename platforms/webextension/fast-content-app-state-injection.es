...
/**
 * Implement a mechanism which allows to inject the state of WDP's background
 ...
 * how it looks like) injects the WDP global (directly on the `window`
 * object), which contains the state of the App (are modules enabled, etc.).
 *
 ...
 *
 * 1. If the main content script is injected first, it will check if the WDP
 ...
 *
 * 2. The secondary content-script will check if the `runContent` global exists on
...
/**
 * Return a self-executing function (as a string) which will inject `WDP`
 * global with `wdp` as a value (or trigger injection of content scripts if
 * `runContent` global is already defined). It is used to update the status of
 * WDP' background modules as seen by content scripts. This information is
 * required to perform injection of modules' content script because we need to
 * know if their background is enabled or not.
 */
function generateContentScript(wdp) {
  return `(function () {
  window.WDP = ${JSON.stringify(wdp)};
  if (window.runContent !== undefined) {
    window.runContent(window.WDP);
    window.runContent = undefined;
  }
}());`;
}

/**
 * Get status of App from API and return a WDP-like object to be injected in
 * content scripts (as `window.WDP`). The object should have a similar shape
 * to the WDP global used in background pages (you know, the one you use in
 * the browser console to debug the extension).
 */
function createWDPObject(app) {
  return {
    app: app.status(),
  };
}
...
class FirefoxInjection {
  ...
  async setWDPGlobal(wdp) {
    ...
    this.scriptRegistration = await browser.contentScripts.register({
      ...
      js: [
        {
          code: generateContentScript(wdp),
        },
      ],
...
class ChromeInjection {
  constructor() {
    ...
    // inject a dynamic content script as soon as possible and set the WDP
  ...
  setWDPGlobal(wdp) {
    this.code = generateContentScript(wdp);
  }
}

export default class FastContentAppStateInjecter {
  constructor() {
    this.scriptInjector = isContentScriptsSupported()
  ...
  shareAppState(app) {
    // The function `setWDPGlobal` isn't concurrency safe.
    ...
    const thisInstance = this._sharingQueue
      .then(() => this.scriptInjector.setWDPGlobal(createWDPObject(app)))
      .catch((e) =>
        window.console.error("ERROR in shareAppState to content scripts:", e)
      )
      .then(() => {
