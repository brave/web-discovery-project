...
function interpol(str, req) {
  ...
  if (interpolStartIndex !== -1) {
    const interpolEndIndex = str.indexOf(
      "}",
      interpolStartIndex + interpolPrefix.length
    );
    if (interpolEndIndex === -1) {
      return str;
    }

    const attribute = str.substring(
      interpolStartIndex + interpolPrefix.length,
      interpolEndIndex
    );
...
(function createApp(port = 3000) {
  ...
  app.post("/mock", async (request, response) => {
    const { path, result, headers, directories, status, timeout } =
      request.body;
      ...
      if (result === "{Transparent.gif}") {
        ...
        res.end(
          Buffer.from([
            ...
            255, 33, 249, 4, 1, 0, 0, 0, 0, 44, 0, 0, 0, 0, 1, 0, 1, 0, 0, 2, 1,
            68, 0, 59,
          ]),
          "binary"
        );
