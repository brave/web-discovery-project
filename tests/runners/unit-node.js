...
log(JSON.stringify(buildConfig));
...
const baseDir = buildConfig.testsBasePath || "./build";

log(`baseDir ${baseDir}`);

const baseURL = baseDir + (buildConfig.platform === "mobile" ? "/dev" : "");
...
function describeModule(moduleName, loadDeps, testFn) {
  ...
  localSystem.config({
    ...
    baseURL,
    meta: {
      "*": { format: buildConfig.format === "common" ? "cjs" : "register" },
    },
  });
  ...
  function loadModules() {
    ...
    return Promise.all(
      Object.keys(depsRewrite).map(function (dep) {
        let depName;
