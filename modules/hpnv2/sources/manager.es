...
import {
  ...
  InitSignerError,
  OldVersionError,
  ClockOutOfSyncWhileJoining,
} from "./errors";
import {
  formatDate,
  formatHoursAsYYYYMMDD,
  reflectPromise,
  encodeWithPadding,
} from "./utils";
...
function ppState(state) {
  return (
    ["READY", "UNINITIALIZED", "INIT_PENDING", "DESTROYED"][state] || "<??>"
  );
}
...
export class InitState {
  ...
  isUnloaded() {
    return (
      this.state === InitState.UNINITIALIZED ||
      this.state === InitState.DESTROYED
    );
  }
 ...
 * (including all messages generated by Web Discovery Project) are signed and
...
export default class Manager {
  _initDefaults() {
    ...
    this.star = new Star();
    this.endpoints = new Endpoints({ star: this.star });
  ...
  async init() {
    ...
    await this.star.init(); // Initialize internal WASM module
  ...
  async updateConfig({
    ...
    if (!ok) {
      ...
      if (oldGroupKeys.length > 0) {
        ...
        if (newGroupPubKeys.length === 1) {
          ...
          this.log(
            "Delay joining the latest group",
            latestKey,
            "by",
    ...
    if (inSync) {
      ...
      const purgeOldPubKeys = this.db
        .purgeOldPubKeys(hoursSinceEpoch)
        .catch((e) => {
  ...
  getPublicKey(date) {
    const smaller = Object.keys(this.groupPubKeys)
      .sort()
      .filter((x) => x <= date);
  ...
  async initUserPK() {
    ...
    this.userPK = await importRSAKey(
      this.userPK64,
      false,
      "SHA-256",
      "RSASSA-PKCS1-v1_5"
  ...
  async joinGroup(date) {
    ...
    const challengeStr = JSON.stringify([
      this.publicKeyB64,
      toBase64(groupPubKey),
    ]);
    const challenge = await sha256(challengeStr, "bin");
    const { gsk, joinmsg } = data.gsk
      ? data
      : await this.signer.startJoin(challenge);
  ...
  async send(
    ...
    try {
      ...
      if (initState.isUnloaded()) {
        ...
        const info =
          "hpnv2 was stopped. The pending message could not be sent.";
        logger.warn(info);
  ...
  async _signMessage(
    ...
    let cnt;
    try {
      cnt = await this.db.consumeFreshCounter(tag, hours + period, limit);
    } catch (e) {
      if (skipQuotaCheck) {
        cnt = 0;
      } else {
  ...
  static get clearText() {
    return (
      prefs.get("hpnv2.cleartext", false) || prefs.get("hpnv2.plaintext", false)
    );
  }
  ...
  async _send(
    ...
    if (!signedMsg) {
      const pretag = [action, period, limit, dig, hours];
  ...
  static checkGroupPublicKeys(newGroupKeys, newPubKeys, oldKeys) {
    ...
    if (oldKeys) {
      ...
      const check = (x) =>
        ...
        x > largestOldDate ||
        (oldKeys[x] &&
          toBase64(oldKeys[x].groupPubKey) === newGroupKeys[x] &&
          (!oldKeys[x].pubKey ||
            toBase64(oldKeys[x].pubKey) === newPubKeys[x]));
