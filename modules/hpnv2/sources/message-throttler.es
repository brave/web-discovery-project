...
export default class MessageThrottler {
  ...
  async startRequest(msg, trustedClock) {
    ...
    if (delay > 0) {
      ...
      logger.log(
        `Too many messages (action=${msg.action}). ` +
  ...
  updateConfig(trafficHints, sourceMap) {
    try {
      ...
      if (
        _settings.minUptimeForThrottle < 0 ||
        _settings.safeDelayAbsolute < 0 ||
  ...
  computeRecommendedDelay(actionGroup, trustedClock) {
    ...
    if (nearMidnight) {
      rule =
        rule || this.rules.midnight[actionGroup] || this.rules.midnight.default;
    }
    ...
    const delay =
      (minWeight * minDelay + maxWeight * maxDelay) / (minWeight + maxWeight);
    ...
    const maxDelayBasedOnUptime =
      this.settings.safeDelayAbsolute +
