...
export function registerContentScript(spec) {
  ...
  if (
    spec.excludeMatches !== undefined &&
    Array.isArray(spec.excludeMatches) === false
  ) {
    console.error(
 ...
 * found in the `WDP` argument and is used to know which modules are enabled.
 ...
 */
export function runContentScripts(window, chrome, WDP) {
  const contentScriptActions = {};

  for (const spec of CONTENT_SCRIPTS) {
    const { module, js } = spec;
    const { isEnabled } = WDP.app.modules[module] || { isEnabled: false };

    if (isEnabled && shouldLoadScript(window, spec)) {
      for (const script of js) {
        try {
          const actions = script(window, chrome, WDP) || {};
          contentScriptActions[module] = {
            ...contentScriptActions[module],
            ...actions,
          };
        } catch (e) {
          console.error(`WDP content-script failed: ${e} ${e.stack}`);
        }
      }
    }
  }
