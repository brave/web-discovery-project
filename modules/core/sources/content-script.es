...
/**
 * This file is the entry-point for content scripts for the extension. At build
 * time, it will bundle the code from `content.es` for each module enabled in
 * the config (or nothing if `content.es` does not exist for a given module);
 ...
 *  manifest for the extension in `specific/web-discovery-project/manifest.json`;
 *  which means it will be injected in every page.
 *
