...
export async function enableRequestSanitizer() {
  ...
  HAS_RIGHT_PERMISSIONS = await PermissionManager.contains([
    PermissionManager.PERMISSIONS.WEB_REQUEST,
    PermissionManager.PERMISSIONS.WEB_REQUEST_BLOCKING,
  ]);
  ...
  webRequest.onBeforeSendHeaders.addListener(safeFilter, networkFilters, [
    "blocking",
    "requestHeaders",
  ]);
}
