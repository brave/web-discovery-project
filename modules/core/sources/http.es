...
} from "../platform/xmlhttprequest";
import {
  fetch as _fetch,
  fetchArrayBuffer as _fetchArrayBuffer,
...
export function defaultHttpHandler(
  ...
  req.onload = () => {
    ...
    if (
      statusClass === 2 ||
      statusClass === 3 ||
      statusClass === 0 /* local files */
    ) {
      if (callback) {
        callback(req);
      }
    } else {
      const error = `loaded with non-200 ${url} (status=${req.status} ${req.statusText}) WDP.httpHandler`;
      console.log(error);
      if (onerror) {
        onerror(error);
      }
  ...
  req.onerror = () => {
    const error = `error loading ${url} (status=${req.status} ${req.statusText}) WDP.httpHandler`;
    console.log(error);
    if (onerror) {
      onerror(error);
    }
  ...
  req.ontimeout = () => {
    const error = `timeout for ${url} WDP.httpHandler`;
    console.log(error);
    if (onerror) {
      onerror(error);
    }
