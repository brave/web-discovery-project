...
export default describeModule(
  ...
  () => {
    describe("MessageQueue", function () {
      ...
      it("Resolve messages in order of insertion", () =>
        new Promise((resolve, reject) => {
          ...
          const queue = MessageQueue("tests", (data) =>
            new Promise((res) => {
      ...
      it("should process all messages in the queue", function () {
        ...
        const queue = MessageQueue("test", () => {
          processedMessages += 1;
        });

        return Promise.all([queue.push(() => {}), queue.push(() => {})]).then(
          () => {
            expect(queue.getSize()).to.equal(0);
      ...
      it("should sequentialize the execution of messages", function () {
        ...
        return Promise.all([queue.push(p1), queue.push(p2)]).then(() => {
