...
export function parsePublicKeyPem(key_: string): string {
  ...
  if (!key.startsWith(START_TAG) || !key.endsWith(END_TAG)) {
    throw new Error(
      `Expected PEM format (${START_TAG} ... ${END_TAG}), but got: ${key_}`
    );
  }
  return key
    .slice(START_TAG.length, key.length - END_TAG.length)
    .replace(/\s+/g, "");
}
...
export default class SignatureVerifier {
  ...
  constructor({
    ...
    if (this.insecure) {
      logger.warn(
        `Signature verification will not be enforced for URL ${resourceUrl}. ` +
    ...
    this._pendingInit = (async () => {
      try {
        ...
        logger.info(
          "Successfully loaded the public key for verifying signatures signed with",
          publicKeyName
        );
      } catch (e) {
        ...
        logger.error(
          `crypto.subtle.importKey failed for key ${publicKeyName} (details: ${e})`
        );
        if (!insecure) {
          throw new Error(
            `Failed to import the key ${publicKeyName} (error: ${e}).`
          );
        }
        logger.info(
  ...
  async checkSignature(
    ...
    if (!goodSignature && this.insecure) {
      logger.info(
        '"insecure" flag is set. Ignore signature verification error for URL:',
        this.signatureUrl
      );
      return true;
    }
  ...
  async _checkSignature(
    ...
    try {
      ...
      logger.warn(
        "Signature is not properly encoded. Expected base64 but got:",
        signature
      );
      return false;
    }
    ...
    try {
      ...
      throw new Error(
        `Unable to verify signature (crypto operations not supported). Error message: ${e}`
      );
    }
  }
}
