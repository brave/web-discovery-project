 ...
 *
 * 1) client-side:
 *    The code in the extension should not make too strong assumptions about
 *    the rules themself. Unnecessary assumptions can make it later more
 *    difficult to push new rules. Also be prepared to handle errors
 *    gracefully.
 *
 * 2) server-side:
...
export class ContentExtractor {
  /**
   * Note: In production, _WebDiscoveryProject will be the global WebDiscoveryProject object.
   */
  constructor(_WebDiscoveryProject) {
    this._WebDiscoveryProject = _WebDiscoveryProject;
    this.msgType = _WebDiscoveryProject.msgType;
    ...
    // TODO: maskURL depends heavily on WebDiscoveryProject functionality.
    // Could also be extracted to another file, but that is a
    // non-trivial change. For now leave the dependence to WebDiscoveryProject.
    this.refineFuncMappings = {
      splitF: refineSplitFunc,
      parseU: refineParseURIFunc,
      maskU: (url) => this._WebDiscoveryProject.maskURL(url),
      json: _jsonPath,
    };
  ...
  checkURL(pageContent, url, ruleset) {
    ...
    logger.debug("PCN: checking URL", url, "against ruleset", ruleset);
    for (let i = 0; i < rArray.length; i += 1) {
      if (rArray[i].test(url)) {
        logger.debug("PCN: URL", url, `matches #${i}`);
  ...
  _createAnonSearchQuery(hostname, query, searchEngineIdx) {
    const template =
      this.patterns.strict.queryTemplate[String(searchEngineIdx)] || {};
  ...
  _checkSearchURL(url, ruleset) {
    ...
    logger.debug("_checkSearchURL", url, ruleset, patterns);
    for (let i = 0; i < rArray.length; i += 1) {
      if (rArray[i].test(url)) {
        if (searchEngines.indexOf(String(i)) !== -1) {
          return i;
  ...
  _extractContent(ind, cd, url, baseURI, ruleset) {
    logger.debug("_extractContent", url);
    const scrapeResults = {};
   ...
   *   will have to pass the wdp sanitizer heuristics
    ...
    try {
      ...
      if (payloadRules.type === "single" && payloadRules.results === "single") {
        scrapeResults[key].forEach((e) => {
          e.ctry = this._WebDiscoveryProject.getCountryCode();
          this._sendMessageIfAllFieldsAreSet(payloadRules, e);
        });
      } else if (
        payloadRules.type === "single" &&
        payloadRules.results === "custom"
      ) {
        ...
        payloadRules.type === "query" &&
        payloadRules.results === "clustered"
      ) {
        const payload = {};
        payloadRules.fields.forEach((e) => {
        ...
        payloadRules.type === "query" &&
        payloadRules.results === "scattered"
      ) {
      ...
      if (this._WebDiscoveryProject.debug) {
        logger.warn(
          "_processExtractedData failed (scrapeResults:",
          scrapeResults,
          ", key:",
          key,
          ", ruleset:",
  ...
  tryExtractBraveSerpQuery(url) {
    ...
    return isBraveSearch && parse(url).searchParams.get("q");
  }
}
