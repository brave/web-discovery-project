...
export default class DoublefetchHandler {
  ...
  _purgeObsoleteRequests(requestStartedAt) {
    while (
      ...
      requestStartedAt - this._pendingRequests[0].ts >
  ...
  _createOnBeforeSendHeadersHandler() {
    // List of headers that should be removed before sending out the
    ...
    return {
      name: "web-discovery-project.stripSensitiveHeadersInDoublefetch",
      spec: "blocking",
      fn: (request, response) => {
        const matchingPendingEvent =
          this._correlatePendingDoublefetchRequest(request);
  ...
  _createOnHeadersReceivedHandler() {
    return {
      name: "web-discovery-project.preventExpensiveDoublefetchRequests",
      spec: "blocking",
      fn: (request, response) => {
        const matchingPendingEvent =
          this._correlatePendingDoublefetchRequest(request);
  ...
  _createOnCompletedHandler() {
    return {
      name: "web-discovery-project.cacheDnsResolution",
      spec: "blocking",
      fn: (request) => {
        if (!request.ip) {
          return;
  ...
  init() {
    this._pendingInit = this._pendingInit.catch(logger.debug).then(() => {
      ...
      const pendingLoad = this._initPipeline()
        .then(() => {
   ...
   */
  _makeRequestAndWaitForHandlers(
    url,
    entry,
