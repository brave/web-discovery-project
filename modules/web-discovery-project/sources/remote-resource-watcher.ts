...
function isContentEqual(
  x: string | Uint8Array | null,
  y?: string | Uint8Array | null
) {
  if (x === null || y === null) {
...
export class RemoteResourceWatcher {
  ...
  constructor(options_: {
    ...
    const nonEmpty = (x: any, reason: string) => {
      if (!x) {
    ...
    this.id = nonEmpty(options.resource.id, "resource.id");
    ...
    this.storage = new Storage(["wdp", this.moduleName]);
    ...
    try {
      ...
      try {
        ...
        if (this.verifiedContent) {
          this.contentCache.update(this.verifiedContent, {
        ...
        logger.warn(
          "Failed to load resources from disk. Starting with empty state.",
          e
        );
      }
      ...
      this.earlyRetryTimer = null;
      this.defaultRefreshTimer = pacemaker.everyFewMinutes(
      ...
      if (this.contentCache.isTooOld({ byMoreThan: 24 * HOUR })) {
        logger.info(
          "Resource is too old. Forcing immediate refresh for",
          this.resourceUrl
        );
        // pacemaker.setTimeout(this.forceUpdate.bind(this), 10000);
        pacemaker.nextIdle(this.forceUpdate.bind(this)); // NOTE: nextIdle could take a very long time
      }
    } finally {
      initFinished();
      this.initInProgress = null;
  ...
  async _checkUpdates() {
    ...
    try {
      ...
      this.resourceOutdated =
        this.resourceOutdated || this.contentCache.isTooOld();
      ...
      if (this.verifiedContentNeedsToBePersisted) {
        try {
          ...
          logger.info(
            "Failed to write verified resources to disk. Retrying later...",
            e
          );
        }
      }
  ...
  _scheduleEarlyRetryIfNecessary() {
    ...
    if (retryRequests && !this.earlyRetryTimer && this.initialized) {
      ...
      if (cooldown && cooldown < 1 * HOUR) {
        ...
        this.earlyRetryTimer = pacemaker.setTimeout(
          this._checkUpdates.bind(this),
          cooldown
        );
      }
  ...
  async _loadFromDisk() {
    try {
      ...
      logger.warn(
        `Could not load resource "${this.moduleName}:${this.id}" from disk. Assuming this is a fresh profile, otherwise, this should not happen.`
      );
      return null;
    }
  }
  ...
  _notifyObservers(verifiedContent?: string | Uint8Array) {
    if (verifiedContent !== null && verifiedContent !== undefined) {
      try {
        ...
        logger.warn(
          `onUpdate callback failed for resource ${this.resourceUrl}`,
          e
        );
      }
    }
