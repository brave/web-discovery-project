    ...
    this.fetch = (url, { signal }) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
          if (
            error ||
            (networkErrorRate > 0.0 && networkErrorRate >= Math.random())
          ) {
            resolve({
              ok: false,
...
function expectNoBursts(events, maxEvents, interval) {
  ...
  for (let end = 0; end < events.length; end += 1) {
    ...
    if (eventsInInterval > maxEvents) {
      chai.assert.fail(
        `Expected no more than ${maxEvents} events within ${
          interval / 1000
        } seconds ` +
...
export default describeModule(
  "web-discovery-project/remote-resource-watcher",
  () => ({
    "core/logger": {
      default: {
        get() {
    ...
    describe("#RemoteResourceWatcher", function () {
      ...
      [
        ...
        describe(`(network: ${JSON.stringify(networkSettings)})`, function () {
          const perfectNetwork =
            networkSettings.minLatencyInMs === 0 &&
          ...
          it("should load patterns on the first startup", async function () {
            ...
            const stateIsEventuallyPersistedOnDisk = new Promise(
              (resolve, reject) => {
                uut.storage.onSave = (persistedContent) => {
                  try {
                    if (
                      persistedContent &&
                      Buffer.from(persistedContent, "utf8").toString() ===
                        CONTENT_FROM_NETWORK
                    ) {
                      resolve();
                      return;
                    }
            ...
            options.signature.verifier = {
              async checkSignature(message, signature) {
                ...
                return (
                  msg.type === "content" &&
                  sig.type === "signature" &&
                  msg.bucket === sig.bucket &&
                  GOOD_BUCKETS.includes(msg.bucket)
                );
              },
            };
