  ...
  describe("WebrequestPipeline tests", () => {
    ...
    describe("details", () => {
      it("main_frame", async () => {
        ...
        await testServer.registerPathHandler(getSuffix(), {
      ...
      it("sub_frame", async () => {
        addPipeline(collectRequestDetails);
        await Promise.all([
          testServer.registerPathHandler(getSuffix("iframe"), {
    ...
    it.skip("Beacon in iframe", async () => {
      await addPipeline((ctx) => {
        collectRequestDetails(ctx);
      });
    ...
    if (!isChromium) {
      describe("Service workers", () => {
        ...
        function setupServiceWorker() {
          return Promise.all([
            testServer.registerPathHandler(
              getSuffix(`${testScope}/service-worker-test.html`),
              {
                result: `<!DOCTYPE html>
              <html>
                <body>
        ...
        it("correct tabUrl when page load intercepted by service worker", async () => {
          ...
          updateTab(tabId, {
            url: getUrl(`${testScope}/service-worker-from-cache.html`),
          });
          await waitFor(
            () =>
              expect(
