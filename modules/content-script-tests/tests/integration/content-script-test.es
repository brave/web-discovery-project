  ...
  describe("registerContentScript", function () {
    ...
    context("module is enabled", () => {
      ...
      xit("can call content scripts actions", async () => {
        ...
        const callContentAction = () =>
          app.modules.core.background.actions.callContentAction(
        ...
        const promises = [];
        const receivingEndDoesNotExist =
        ...
        while (promises.length === 0) {
          try {
            ...
            if (
              JSON.stringify(ex).includes(receivingEndDoesNotExist) === false &&
