...
module.exports = (program) => {
  program
    ...
    .option(
      ...
      "firefox profile name or absolute path (web-ext)"
    )
    ...
    .action((configPath, options) => {
      ...
      process.env.SOURCE_MAPS = options.maps && !isProduction;
      process.env.SOURCE_DEBUG = options.debug;
      process.env.INCLUDE_TESTS =
        options.includeTests ||
        ((configPath || process.env.CONFIG_PATH).includes("/ci/")
          ? "true"
          : "");
      ...
      getExtensionVersion(options.version, CONFIG)
        .then((version) => {
          ...
          const watcher = createBuildWatcher(
            ...
            async () => {
              try {
                if (
                  ["webextension"].indexOf(CONFIG.platform) >= 0 &&
                  ...
                  if (extensionRunner) {
                    ...
                    const FirefoxBrowser =
                      require("../../tests/runners/launchers/firefox-web-ext").Browser;
